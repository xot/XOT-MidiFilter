...
Length
: Length in milliseconds (integer) or in Max time format syntax (e.g. '4n' or '16nd'; in this the delay must be enclosed in single quotes). Defines the length of the note to be output (a note off will be sent that much time later). See above for what happens when length equals 0.

Delay
: Delay in milliseconds (integer) or in Max time format syntax (e.g. '4n' or '16nd'; in this the delay must be enclosed in single quotes). Defines the delay before the note is actually output (the corresponding note off is similarly delayed).
...
- `floor(f)`: the largest integer smaller than or equal to f.
...
This will split an incoming note into two notes, 16th note apart.

Set Sync to In and Through to On. (Enabling through creates the repetition.)

Gate
: `true`

Note
: `N[0]`; use the original note

Velocity
: `V[0]`; use the original velocity. Using `V[0]-20` will make the second node slightly less loud.

Length
: `'32n'`; make the length half a 16-th note

Delay
: `'16n'`; delay for 16th of a note
